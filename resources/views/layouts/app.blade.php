...
<div class="flex h-screen overflow-hidden">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-blue-900 text-white flex flex-col transition-transform duration-300 ease-in-out"
        ...
        <nav class="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
            ...
            @php
                $tenantPlan = auth()->user()?->tenant?->plan;
                $hasExport = in_array($tenantPlan, ['starter','pro','cabinet']);
            @endphp
            @if($hasExport)
        ...
        {{-- Quota bar --}}
        @php $tenantForQuota = auth()->user()?->tenant; @endphp
        @if($tenantForQuota && $tenantForQuota->quota_factures_mensuel > 0)
            @php
                $used = method_exists($tenantForQuota, 'facturesCeMois') ? $tenantForQuota->facturesCeMois() : 0;
    ...
    <div class="flex-1 flex flex-col min-h-screen lg:ml-64 transition-all duration-300">
        ...
        <header class="bg-white border-b border-gray-200 h-14 flex items-center px-4 sm:px-6 sticky top-0 z-30 gap-4">
            ...
            {{-- Plan badge (toujours frais depuis la DB) --}}
            @php $tenantBadge = auth()->user()?->tenant()->first(); @endphp
            @if($tenantBadge)
