...
<div class="max-w-3xl mx-auto space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <span class="text-xs px-2 py-0.5 rounded-full font-medium
            {{ $tenant->statut==='actif'?'bg-green-100 text-green-700':($tenant->statut==='trial'?'bg-amber-100 text-amber-700':($tenant->statut==='suspendu'?'bg-red-100 text-red-700':'bg-gray-100 text-gray-500')) }}">
    ...
    <form method="POST" action="{{ route('admin.tenants.update', $tenant) }}" class="space-y-5">
        ...
        <div class="flex items-center gap-3">
            <button type="submit"
                    class="bg-emerald-800 text-white px-6 py-2.5 rounded-lg font-semibold text-sm hover:bg-emerald-900 transition shadow-sm">
